-- Public initialization template; original local SQL is ignored.
-- Set @admin_password_hash in this MySQL session to your own BCrypt hash before SOURCE.
-- Existing tables are dropped: use a new empty development database only.
-- ============================================================
-- 若依极简空壳模板 SQL
-- 仅保留框架启动和登录所需的最少表和数据
...
insert into sys_dept values(100, 0, '0', '总公司', 0, 'admin', '', '', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(101, 100, '0,100', '默认部门', 1, 'admin', '', '', '0', '0', 'admin', sysdate(), '', null);
...
-- 默认密码: admin123
insert into sys_user (user_id, dept_id, user_name, nick_name, user_type, password, status, del_flag, create_by, create_time) values (1, 101, 'admin', 'Administrator', '00', @admin_password_hash, '0', '0', 'admin', sysdate());
...
insert into sys_config values(2, '用户管理-账号初始密码',      'sys.user.initPassword', '',     'Y', 'admin', sysdate(), '', null, '');
